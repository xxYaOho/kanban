...
| `tester.submit-cases` | tester | test-cases file exists; case scope references known developers | `test-cases-*` | `tester.case_document=<file>` |
| `tester.submit-report` | tester | test report exists; report links reviewed case document; pass covers all developer entries | `test-*` | `tester.report=<file>`, `pass/fail`, developer `done` on pass |
| `owner.closeout` | owner | all developers done; tester done with valid pass evidence; required integrator evidence done; closeout file exists | `owner-closeout-*` | `owner.closeout=<file>`, task `status=done` |
| `integrator.submit-integration-report` | integrator | all developers done; tester pass evidence valid; integration report exists | `integration-*` | `integrator.report=<file>`, `merged/conflicts/status` |
...
benchmark 不属于 skill 运行时。主 benchmark 放在 repo 根部的 `benchmarks/case_replica-blenderhunt/`,用于需要时验证 vNext 设计。历史 origin / vNext run 归档在 `benchmarks/archives/`,摘要见 `benchmarks/benchmark-archive-report-20260617.md`。
...
- 文件系统记录仍能恢复完整过程。
- Human 介入点保持清晰。
- 每次 run 使用固定模板和 `scripts/validate-run.ts` 校验输出路径、metrics 字段和视觉对比证据。
