...
1. **定位任务**:读取 kanban,按 UUID(或短前缀)找到任务,回显当前人工字段(status / description / plan / repo / 所有 worktree 的 role+action)
...
6. **`status → planned` 的提升校验**(在锁内,写前):
   - `plan` 文件存在且非空字节
   - `worktree` 至少一个条目
   - 每个 worktree 有 `role ∈ {developer,reviewer,test}` 和 非空 `action`
   不满足则拒绝写入,列出缺失项:
   ```
...
- value 有空格时用引号;JSON 用单引号包外层,避免 shell 转义
- 多个操作**原子提交**(同一个 `withKanbanLock` 内),任一非法则全部不生效
...
**不支持**撤销上一步。每次提交前的 diff + 二次确认已足够防误操作。如需恢复历史,请用 git 管 `~/.kanban/kanban.jsonc`(建议 `cd ~/.kanban && git init`)。
